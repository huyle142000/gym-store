...
import { getProductSearch, getProduct, deleteProduct } from '@/store/app/product'
...
const ProductManagement = () => {
    ...
    const { useQueryWrapper, useMutate } = RequestProcessor();

    // Query to get a list of products
    const { data: productList }: any = useQueryWrapper<Product[]>(
        'products',
        getProduct
    );

    const { data: categoryList }: any = useQueryWrapper<Product[]>(
        'categories',
        getCategory
    );
            ...
            <Grid container spacing={6} sx={{ width: '100%' }}>
                ...
                <Grid item xs={12}>
                    <Box className="white-box" sx={{ border: '1px solid rgba(76, 78, 100, 0.12)', borderRadius: '10px' }}>
                        ...
                        <Box sx={{ height: "500px" }}>
                            <DataGrid
                                sortingMode='client'
                                rows={productList !== undefined ? productList : []}
                                columns={columns}
                                initialState={{
                                    pagination: {
                                        paginationModel: {
                ...
                {drawer && <Drawer
                    ...
                    <Box id="drawer-content" sx={{ maxHeight: '100%', overflowY: 'auto' }}>
                        <AddEditProduct onCloseDrawer={() => onCloseDrawer()} panelId={selectedId} drawer={drawer} storeProductCategory={categoryList} onSearch={onSearch}  />
                    </Box>
                </Drawer>}
                {/* PopUpDetele */}
                <NotifyFunction
